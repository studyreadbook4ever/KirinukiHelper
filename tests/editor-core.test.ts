...
test("AI 처리 경고는 프로젝트 전체 상한에서 잘라 저장·병합한다", () => {
  ...
  assert.equal(required(merged.at(-1)).code, "TRIMMED_WARNING_COUNT");
  ...
  assert.equal(
    required(normalized.ai.warnings.at(-1)).code,
    "TRIMMED_WARNING_COUNT"
  );
});

test("v1 프로젝트를 컷·자막 수정 상태를 잃지 않고 v3로 이관한다", () => {
  const current: EditorProject = createEditorProjectFromCapture(captureState, {
    id: "legacy-project",
    createdAt: "2026-07-27T09:00:00.000Z"
  });
  const legacyCue: EditorSubtitleCue = createSubtitleCue(current, {
    id: "legacy-cue",
    clipId: "clip-first",
    startOffsetMs: 700,
    endOffsetMs: 2_100,
  ...
  const cueWithoutV2Fields = {
    ...legacyCue,
    x: 0.31,
    y: 0.73
  } as Record<string, unknown>;
  delete cueWithoutV2Fields.lane;
  delete cueWithoutV2Fields.color;
  const legacy = {
    ...current,
  ...
  delete legacy.subtitleLaneCount;
  delete legacy.suppressedSelections;
  delete legacy.audioRegions;
  delete legacy.imageAssets;
  delete legacy.selectedImageAssetId;
  delete legacy.selectedAudioRegionId;
...
test("v2 프로젝트를 기존 자막·음성 선택 상태를 보존해 v3로 이관한다", () => {
  let current: EditorProject = createEditorProjectFromCapture(captureState, {
    id: "v2-project",
  ...
  const legacyCurrent = { ...current } as Partial<EditorProject>;
  delete legacyCurrent.imageAssets;
  delete legacyCurrent.selectedImageAssetId;

  const migrated = normalizeEditorProject(legacyCurrent);
  assert.ok(migrated);
  assert.equal(migrated.schema, EDITOR_SCHEMA);
...
test("저장된 자동 AI 자막만 기본 위치로 이관하고 사람 소유 위치는 보존한다", () => {
  ...
  const once = required(normalizeEditorProject(structuredClone(raw)));
  const automatic = required(
    once.subtitles.find((cue) => cue.id === "automatic-ai")
  );
  const reviewed = required(
    once.subtitles.find((cue) => cue.id === "reviewed-ai")
  );
  const manual = required(
    once.subtitles.find((cue) => cue.id === "manual-caption")
  );
  const automaticRemoteMeta = required(automatic.remoteMeta);
  const reviewedRemoteMeta = required(reviewed.remoteMeta);
  assert.deepEqual(
    [automatic.x, automatic.y, automaticRemoteMeta.placement],
    [0.5, 0.84, "bottom"]
  );
  assert.deepEqual(
    [reviewed.x, reviewed.y, reviewedRemoteMeta.placement],
    [0.31, 0.27, "top"]
  );
  assert.deepEqual([manual.x, manual.y], [0.76, 0.44]);
  assert.equal(Object.hasOwn(manual, "remoteMeta"), false);
...
test("명시적 전체 정렬은 AI 자막 위치만 초기화하고 수동 자막은 건드리지 않는다", () => {
  ...
  const reviewed = required(
    reset.subtitles.find((cue) => cue.id === "reviewed-ai")
  );
  const manual = required(
    reset.subtitles.find((cue) => cue.id === "manual-caption")
  );
  const reviewedRemoteMeta = required(reviewed.remoteMeta);
  assert.deepEqual(
    [reviewed.x, reviewed.y, reviewedRemoteMeta.placement],
...
test("자막 색상 레지스터는 흰색을 제외한 최근 고유 색상 5개만 MRU로 보존한다", () => {
  ...
  for (const color of [
    ...
    project = required(rememberSubtitleColor(project, color));
  }
  assert.deepEqual(project.recentSubtitleColors, [
    "#00ffff",
    "#ff00ff",
  ...
  project = required(rememberSubtitleColor(project, "#00F"));
  assert.deepEqual(project.recentSubtitleColors, [
    "#0000ff",
    "#00ffff",
    "#ff00ff",
...
test("로컬 AI 초벌은 기존 사람·AI 자막을 그대로 두고 충돌 없는 별도 레인에 한 번만 추가한다", () => {
  ...
  assert.equal(
    required(
      repeated.subtitles.find((cue) => cue.id === "codex-first-pass-1")
    ).lane,
    2
  );
...
test("동시에 말하는 원격 화자는 서로 다른 자막 레인에 모두 보존한다", () => {
  ...
  assert.equal(required(
    project.subtitles.find((cue) => cue.id === "simultaneous-main")
  ).y, 0.84);
  assert.equal(required(
    project.subtitles.find((cue) => cue.id === "simultaneous-guest")
  ).y, 0.84);
  assert.equal(required(
    project.subtitles.find((cue) => cue.id === "main-followup")
  ).y, 0.84);
});
...
test("컷 trim은 남은 자막의 원본 발화 시각을 보존하고 잘려나간 자막은 제거한다", () => {
  ...
  const kept = required(
    project.subtitles.find((cue) => cue.id === "kept")
  );
...
test("컷 trim은 영상과 같은 원본 시각을 유지하며 현재·후속 컷 자막을 함께 당긴다", () => {
  ...
  const firstCueAfter = required(
    project.subtitles.find((cue) => cue.id === "first-source-cue")
  );
  const secondCueAfter = required(
    project.subtitles.find((cue) => cue.id === "second-source-cue")
  );
  const firstClipAfter = required(
    project.clips.find((clip) => clip.id === "clip-first")
  );
  assert.deepEqual({
...
test("가운데 구간 삭제는 컷을 분할하고 자막·에셋·음성을 원본 시각 기준으로 함께 잇는다", () => {
  ...
  assert.equal(
    required(
      project.clips.find((clip) => clip.id === "clip-second")
    ).timelineStartMs,
    4_625
  );
  ...
  const movedAfterCue = required(
    project.subtitles.find((cue) => cue.id === "after-cue")
  );
  assert.deepEqual({
...
test("여러 컷을 가로지르는 삭제도 뒤 자막의 원본 발화 시각을 보존해 한꺼번에 당긴다", () => {
  ...
  const afterCue = required(
    project.subtitles.find((cue) => cue.id === "after-cross-cut")
  );
  const secondClip = required(project.clips[1]);
  assert.deepEqual({
    clipId: afterCue.clipId,
    startOffsetMs: afterCue.startOffsetMs,
    endOffsetMs: afterCue.endOffsetMs,
    sourceStartMs: secondClip.sourceStartMs + afterCue.startOffsetMs,
...
test("동일한 캡처를 다시 열어도 편집기에서 확장한 컷과 그 자막을 되돌리지 않는다", () => {
  ...
  const first = required(
    merged.clips.find((clip) => clip.id === "clip-first")
  );
