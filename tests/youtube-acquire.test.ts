...
import {
  ...
  requireExecutable,
  runSpawnedCommand
} from "../scripts/acquire-youtube.js";
...
function successfulSpawn(calls: SpawnCall[]): SpawnImplementation {
  ...
  return implementation as unknown as SpawnImplementation;
}
...
test("모든 프로필은 part·무덮어쓰기·단일 영상·최종 경로 출력과 무재인코딩을 고정한다", () => {
  for (const profile of Object.keys(
    YOUTUBE_ACQUISITION_PROFILES
  ) as Array<keyof typeof YOUTUBE_ACQUISITION_PROFILES>) {
    const args = buildYtDlpArgs({
      url: CANONICAL_URL,
      profile,
      outputDir: "/media/output"
