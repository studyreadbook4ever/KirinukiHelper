...
async function createPublicShellFixture(): Promise<string> {
  ...
  for (const relativePath of [
    ...
    await writeFile(
      ...
      await readFile(new URL(relativePath, repositoryPublicShell))
    );
  }
  return root;
}
...
test("공개 shell request target과 route는 다섯 content 경로만 허용한다", () => {
  for (const target of [
    "/",
    "/index.html",
    "/public.css",
...
test("_headers는 HSTS를 포함한 exact 보안 계약만 파싱한다", async () => {
  ...
  const source = await readFile(new URL("_headers", repositoryPublicShell), "utf8");
...
test("공개 shell 서버는 GET·HEAD content와 모든 오류에 같은 보안 헤더를 적용한다", async (t) => {
  ...
  const routes = [
    ...
    ["/public.css?v=3.0.0", "public.css", "text/css; charset=utf-8"],
    [
      "/THIRD_PARTY_NOTICES.md",
      "THIRD_PARTY_NOTICES.md",
...
test("공개 shell startup은 symlink·hardlink·oversize content를 fail-closed한다", async (t) => {
  const roots: string[] = [];
  t.after(async () => {
  ...
  await rm(path.join(finalSymlinkRoot, "public.css"));
  await symlink(outsideCss, path.join(finalSymlinkRoot, "public.css"));
  ...
  await link(
    path.join(hardlinkRoot, "public.css"),
    path.join(hardlinkRoot, "public-copy.css")
  );
  await assert.rejects(
  ...
  await writeFile(
    path.join(oversizeRoot, "public.css"),
  ...
  await writeFile(
    outsideHeaders,
    await readFile(new URL("_headers", repositoryPublicShell))
  );
...
test("공개 배포 checker는 exact HTTPS와 무쿠키·무NEL·무주입 shell만 통과시킨다", async () => {
  const body = await readFile(new URL("index.html", repositoryPublicShell), "utf8");
