...
interface AudSegWorkerEvent {
  data: {
    requestId: string;
    ok: true;
    result: ReturnType<typeof segmentAudSegPcm>;
  };
}

type AudSegWorkerListener = (event: AudSegWorkerEvent) => void;

class FakeAudSegWorker {
  readonly respond: boolean;
  readonly listeners = new Map<string, Set<AudSegWorkerListener>>();
  terminated = false;
  ...
  addEventListener(type: string, listener: AudSegWorkerListener) {
    const listeners = this.listeners.get(type) || new Set();
    listeners.add(listener);
    this.listeners.set(type, listeners);
  }

  removeEventListener(type: string, listener: AudSegWorkerListener) {
    this.listeners.get(type)?.delete(listener);
  }

  postMessage(message: AudSegWorkerRequest) {
    if (!this.respond) {
      return;
    }
    queueMicrotask(() => {
...
test("AudSeg는 upstream과 같은 미세 overshoot를 clamp하고 잘못된 PCM은 거절한다", () => {
  ...
  assert.throws(
    ...
    } as unknown as Parameters<typeof segmentAudSegPcm>[1]),
    /기본값으로 고정/u
  );
});
...
test("AudSeg Worker 경로는 결과 뒤 종료되고 AbortSignal로 즉시 취소된다", async () => {
  ...
  const result = await segmentAudSegPcmInWorker(
    ...
    { workerFactory: () => completedWorker as unknown as Worker }
  );
  assert.equal(result.segments.length, 1);
  ...
  const pending = segmentAudSegPcmInWorker(silence(1), {
    ...
    workerFactory: () => waitingWorker as unknown as Worker
  });
  controller.abort();
