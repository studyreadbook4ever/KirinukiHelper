...
function hasProtocolCode(error: unknown, code: string): boolean {
  return error instanceof CaptionProtocolError && error.code === code;
}
...
test("caption-agent 요청은 스키마·한국어·클립 길이·base64를 엄격히 검증한다", () => {
  ...
  assert.throws(
    ...
    (error) => hasProtocolCode(error, "UNSUPPORTED_LANGUAGE")
  );
  assert.throws(
    () => validateCaptionAgentRequest(protocolRequest({ model: "remote-model" })),
    (error) => hasProtocolCode(error, "INVALID_REQUEST_FIELD")
  );
  assert.throws(
    () => validateCaptionAgentRequest(protocolRequest({ visual: undefined })),
    (error) => hasProtocolCode(error, "UNKNOWN_REQUEST_FIELD")
  );
  assert.throws(
    () => validateCaptionAgentRequest(protocolRequest({
      editorialContext: {
    ...
    (error) => hasProtocolCode(error, "INVALID_EDITORIAL_CONTEXT")
  );
  assert.throws(
    () => validateCaptionAgentRequest(protocolRequest({
...
test("원시 cue와 응답 warning 개수 상한을 처리 전에 강제한다", () => {
  assert.throws(
    ...
    (error) => hasProtocolCode(error, "CAPTION_CUE_LIMIT_EXCEEDED")
  );
  ...
  assert.throws(
    ...
    (error) => hasProtocolCode(error, "INVALID_RESPONSE_WARNINGS")
  );
});
...
test("로컬 Whisper 이외의 응답 provider는 거절한다", () => {
  ...
  assert.throws(
    ...
    (error) => hasProtocolCode(error, "INVALID_RESPONSE_PROVIDER")
  );
});
