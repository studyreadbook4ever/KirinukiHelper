...
} from "../scripts/local-caption-stack.js";
...
test("CLI의 Node 최소 버전은 Windows file identity 수정이 포함된 22.17.0을 정확히 지킨다", () => {
  assert.equal(supportedNodeVersion("22.16.99"), false);
  assert.equal(supportedNodeVersion("22.17.0"), true);
  assert.equal(supportedNodeVersion("22.17.1"), true);
...
test("Linux foreground identity는 proc start tick과 systemd active-like 상태를 정확히 읽는다", () => {
  ...
  assert.equal(commandLineRunsExactCaptionCli({
    commandLine:
      `/usr/bin/node\u0000--import\u0000tsx\u0000${expectedCliPath}\u0000start\u0000--foreground\u0000`,
    processCwd: packageRoot,
    expectedCliPath
  }), true);
  assert.equal(commandLineRunsExactCaptionCli({
    commandLine:
      "/usr/bin/node\u0000--import\u0000tsx\u0000/opt/foreign-clone/scripts/local-caption-stack.ts\u0000start\u0000",
...
test("모든 child 환경은 API 비밀을 제거하고 loopback proxy 우회를 강제한다", () => {
  ...
  assert.match(String(managed.NO_PROXY), /(?:^|,)127\.0\.0\.1(?:,|$)/u);
  assert.match(String(managed.NO_PROXY), /(?:^|,)localhost(?:,|$)/u);
...
test("Whisper 서버 인자는 VAD와 한국어 타임스탬프를 켜고 127.0.0.1만 사용한다", () => {
  ...
  assert.throws(
    () => buildWhisperServerArgs(
      { ...cpuConfig, host: "0.0.0.0" } as unknown as typeof cpuConfig,
      { requestPath }
    ),
    /127\.0\.0\.1/u
  );
...
test("package scripts는 doctor/setup/start/status/stop을 Node CLI로 노출한다", async () => {
  ...
  for (const command of ["doctor", "setup", "start", "status", "stop"]) {
    assert.equal(
      ...
      `node --import tsx scripts/local-caption-stack.ts ${command}`
    );
  }
});
...
test("CLI source는 gateway를 오케스트레이션하되 키를 env·argv에 주입하지 않는다", async () => {
  const source = await readFile(
    path.join(packageRoot, "scripts", "local-caption-stack.ts"),
    "utf8"
  );
