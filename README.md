...
2. 같은 페이지에서 원본을 봅니다. YouTube는 `W` 연결 확인, `E/R` 시작·끝,
   `D/F` ±5초, `Y/U` 0.25·2배속을 웹에서 바로 사용할 수 있습니다. CHZZK·SOOP은
   도우미가 연결돼 있다면 권리 확인 후 `W`로 같은 단축키를 활성화할 수 있고,
   도우미 없이 플레이어 시각을 시작·끝 칸에 직접 입력해도 됩니다. `T`는 다음 구간,
   `Q`는 저장본 확인, `A`는 편집기 열기입니다.
