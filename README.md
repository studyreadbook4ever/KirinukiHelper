...
설치를 안내합니다. 현재 공개 테스트는 Windows x64, Debian/Ubuntu와 Arch Linux x64를 제공하며,
도우미는 별도 작업
...
> 연결·삭제 검증을 통과한 byte만 웹에서 안내합니다. Windows x64는 코드 서명되지
> 않은 GitHub prerelease라 SmartScreen 경고가 표시될 수 있습니다. macOS 설치판은
> 현재 제공하지 않으며 정식 안정판은 별도의
> [데스크톱 바이너리 출시 게이트](legal/DESKTOP_BINARY_RELEASE_GATE.md)를 따릅니다.

## 사용자 흐름

1. 웹사이트에 CHZZK·YouTube·SOOP VOD URL을 붙여넣습니다.
2. 같은 페이지에서 원본을 봅니다. YouTube는 `E/R` 시작·끝, `D/F` ±5초,
   `Y/U` 0.25·2배속을 웹에서 바로 사용할 수 있습니다. CHZZK·SOOP은
   영상 준비 도우미가 공개 HLS를 이 PC의 loopback으로만 연결하고, 웹사이트의 같은
   플레이어에서 `E/R/D/F/Y/U`가 동작합니다. 컷 타임스탬프와 화면 재생 시각은 이
   플레이어의 실제 시계 하나를 공유합니다. 별도 창·수동 시각 입력·연결 키는 없습니다.
   `T`는 다음 구간, `Q`는 저장본 확인, `A`는 편집기 열기입니다.
3. 프로젝트 이름과 이번 사용의 권리·책임 확인란을 입력한 뒤 **편집기 열기**를
   누릅니다. 영상 준비 도구가 없을 때만 OS에 맞는 설치 안내가 한 번 나타납니다.
   Linux 사용자는 시작 화면의 보조 링크에서 GitHub Release 도우미를 미리 받을 수도
   있으며, 다운로드를 누르면 같은 화면이 설치와 연결 상태를 계속 확인합니다.
...
- 로컬 엔진은 브라우저만으로 안정적으로 할 수 없는 공개 VOD 재생 연결·선택 구간
  취득과 검증만 담당합니다. 앱 창, 컷 화면, 프로젝트 목록 또는 편집기를 만들지
  않습니다.
...
현재 웹사이트가 공개 테스트로 제공하는 파일은 다음 세 개입니다.

| 운영체제 | 대상 | 설치 파일 |
|---|---|---|
| Windows | x64 | `Kirinuki-Engine-windows-x64-preview-setup.exe` |
...
현재 macOS, Windows ARM64, Linux ARM64와 모바일 편집은 공개 테스트에서 지원하지 않습니다.
...
Windows preview는 exe 설치가 끝나면 화면 없는 도우미를 시작하고 HKCU 자동 시작을
등록합니다. Linux preview는 deb 또는 pkg.tar.zst 설치 뒤 웹사이트의 **이 PC 연결**이
도우미를 깨우고 자동 시작을 등록합니다. Windows 파일은 서명되지 않았으므로
SmartScreen에서 ‘추가 정보’를 확인해야 할 수 있습니다. 다운로드 안내는 exact
tag-pinned prerelease만 사용하며 SHA-256, 소스·라이선스 안내와 build attestation을
함께 제공합니다.
...
- 시작 화면과 편집기에서 상태를 잃지 않고 전환하는 한국어·영어·일본어 UI
