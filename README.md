...
기존 unpacked Extension ID와 그 프로필의 편집본을 의도적으로 이어야 하는
고급 마이그레이션에서는 두 경로를 **절대경로**로 명시한 채 설정합니다.

```bash
KIRINUKI_EXTENSION_ROOT="/절대/경로/extension" \
KIRINUKI_BROWSER_PROFILE_ROOT="/절대/경로/chromium-profile" \
./setup.sh
```

설치된 `kirinuki` 래퍼는 이 두 선택을 보존합니다. 상대경로·빈 값·앞뒤 공백·
줄바꿈은 조용히 기본값으로 바꾸지 않고 실패합니다. 지정한 Extension 경로에는
현재 repository의 `extension`과 파일 단위로 정확히 같은 최신 빌드가 미리
준비되어 있어야 합니다. 도우미는 편집 identity가 묶인 외부 경로를 자동으로
덮어쓰지 않으며, 누락·변경·남은 구버전 파일이 있으면 fail-closed합니다.
`kirinuki doctor`의 빌드 일치 표시를 확인하세요.
...
1. 치지직 라이브·VOD 또는 YouTube VOD를 재생합니다.
   캡처 카드의 **5초 뒤로 [D]**·**5초 앞으로 [F]**로 원본 위치를 다듬고,
   **0.25× [Y]**·**2× [U]** 버튼으로 현재 플레이어만 느리게 또는 빠르게
   들을 수 있습니다. 위치·속도는 프로젝트에 저장되지 않으며 YouTube 광고
   중에는 원본 시각 보호를 위해 바꾸지 않습니다.
2. 사용할 사건이 시작될 때 **시작 스탬프 → 지금 [E]**을 누릅니다.
3. 사건의 반응이나 결론이 끝날 때 **끝 스탬프 → 지금 [R]**을 누릅니다.
...
10. `영상 → 에셋 → 음성 → 자막` 타임라인에서 이미지·자막·음성을 검수합니다. 영상 중간을 덜어낼 때는 재생헤드에서 `시작 [I]`와 `끝 [O]`을 찍고 **구간 삭제**를 누릅니다. 삭제 뒤 영상과 연결된 에셋·음성·자막은 함께 당겨집니다. 웹 이미지 자체를 복사한 뒤 편집기에서 `Ctrl/Cmd+V`를 누르면 현재 위치에 에셋으로 들어가며 PNG·WebP의 투명 영역도 보존됩니다. 자막 레인은 기본 2개이며 `+`로 늘릴 수 있습니다. **선택 자막 색상** 오른쪽 레지스터는 고정 흰색 `#FFFFFF`와 최근 비흰색 5개를 기억하고 `1`~`6`으로 바로 적용합니다. `J`/`K`는 같은 레인의 이전·다음 자막, `,`/`.`는 이전·다음 컷, `Space`는 재생·일시정지입니다. 타임라인 **자석**으로 자막↔에셋 경계를 가까이 끌어 붙이고, 같은 컷의 둘을 선택한 뒤 **선택 에셋 구간에 정확히 맞춤** 또는 **선택 자막 구간에 정확히 맞춤**으로 양끝을 한 번에 일치시킬 수 있습니다. 구버전 AI 초벌의 화면 위치가 섞여 있으면 **AI 자막 전체를 기본 위치로 정렬**을 한 번 누릅니다.
...
내보내기를 시작하면 현재 프로젝트를 먼저 로컬 복구 초안으로 저장하고 원본
파일이 편집 중 바뀌지 않았는지 다시 확인합니다. 긴 영상은 파일 인코딩과
브라우저 파일 커밋이 모두 끝날 때까지 최대 99%로 표시되며, 실제 커밋 뒤에만
100%가 됩니다. 실패 시 확실히 빈 출력만 정리하고, 기록된 바이트가 있거나
커밋 여부를 확인할 수 없는 파일은 복구 가능성을 위해 지우지 않습니다.
