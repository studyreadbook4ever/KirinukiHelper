...
type ElementReference = ExternalRecord & {
  [ELEMENT_KEY]: string;
};
type TimelinePointerTarget = {
  x: number;
  y: number;
  blockLeft: number;
  blockWidth: number;
...
type PlayheadUiState = {
  value: string | null;
  valueText: string | null;
  paused: boolean;
  previewCurrentTime: number;
  previewDuration: number;
  previewSeekable: Array<{ start: number; end: number }>;
  pointerEvents: Array<{ isTrusted?: boolean } & ExternalRecord>;
};
...
interface ProjectRecord extends ExternalRecord {
  id: string;
  name: string;
...
type MemoryFile = {
  ...
  committed: boolean;
  preexisting: boolean;
  createWritableCalls: ExternalRecord[];
  closeUiSnapshots: CloseUiSnapshot[];
  closeCount: number;
  abortCount: number;
  transactions: WritableOperation[][];
  base64: string;
};
type MemoryDirectorySnapshot = {
  error?: unknown;
  files: MemoryFile[];
  pickerCalls: Array<{ id?: string; mode?: string }>;
  savePickerCalls: ExternalRecord[];
  anchorDownloads: ExternalRecord[];
  getFileHandleCalls: Array<{ name: string; create: boolean }>;
};
type PickerOverride = {
  ok?: boolean;
  error?: unknown;
  directoryPickerType?: string;
  savePickerType?: string;
  contractProbe?: {
    committedText?: string;
    abortPreservedText?: string;
    operations?: WritableOperation[];
    abortOperations?: WritableOperation[];
  };
};
type MediaProbe = {
  format: {
...
function isMediaAssetRecord(value: unknown): value is MediaAssetRecord {
  return (
    isExternalRecord(value)
    && typeof value.name === "string"
    && typeof value.mediaOriginMs === "number"
...
function isSubtitleRecord(value: unknown): value is SubtitleRecord {
  return (
    isExternalRecord(value)
    && typeof value.id === "string"
    ...
    && typeof value.endOffsetMs === "number"
  );
}
...
function isAudioRegionRecord(value: unknown): value is AudioRegionRecord {
  return (
    isExternalRecord(value)
    && typeof value.id === "string"
    && typeof value.clipId === "string"
...
function isProjectRecord(value: unknown): value is ProjectRecord {
  return (
    ...
    && typeof value.id === "string"
    && typeof value.name === "string"
    && (value.mediaAsset === null || isMediaAssetRecord(value.mediaAsset))
    && Array.isArray(value.clips)
    && value.clips.every(isClipRecord)
    && Array.isArray(value.subtitles)
    && value.subtitles.every(isSubtitleRecord)
    && Array.isArray(value.imageAssets)
    && value.imageAssets.every(isImageAssetRecord)
    && (
      value.selectedImageAssetId === undefined
      || value.selectedImageAssetId === null
      || typeof value.selectedImageAssetId === "string"
    )
...
function describeHttpError(payload: unknown, fallback: string): string {
  ...
  if (
    isExternalRecord(payload.value)
...
async function runCommand(
  executable: string,
  args: readonly string[],
  options: { binary: true; timeout?: number }
): Promise<Buffer>;
async function runCommand(
  executable: string,
  args: readonly string[],
  options?: { binary?: false; timeout?: number }
): Promise<string>;
async function runCommand(executable: string, args: readonly string[], {
  binary = false,
  timeout = 60_000
}: { binary?: boolean; timeout?: number } = {}): Promise<string | Buffer> {
  const child = spawnTracked(executable, args, {
    stdio: ["ignore", "pipe", "pipe"]
  });
  const stdout: Buffer[] = [];
  const stderr: Buffer[] = [];
  assert(child.stdout, `${path.basename(executable)} stdout pipe가 없습니다.`);
  assert(child.stderr, `${path.basename(executable)} stderr pipe가 없습니다.`);
...
async function webdriver<T = ExternalRecord>(
  ...
  const payload = readWebDriverEnvelope(await fetchJson(
...
async function cdp<T = ExternalRecord>(
  command: string,
  params: ExternalRecord = {}
): Promise<T> {
  return webdriver<T>("POST", `/session/${sessionId}/goog/cdp/execute`, {
    cmd: command,
    params
  });
}

async function waitUntil<T>(check: () => Promise<T | false> | T | false, description: string, {
  timeout = 20_000,
  interval = 120
...
async function waitForExtensionTarget(debuggerAddress: string, serviceWorkerPath: string) {
  for (let attempt = 0; attempt < 60; attempt += 1) {
    ...
    const target = targets.find((entry): entry is ExtensionTarget => {
...
async function findElement(selector: string): Promise<ElementReference> {
  const element = await webdriver<ExternalRecord>(
    "POST",
...
async function clickTimelineRulerAtClipFraction(clipId: string, fraction = 0.5) {
  const target = await executeSync<TimelinePointerTarget | null>(`
...
async function clickTimelineRulerAtClipMidpoint(clipId: string) {
  return clickTimelineRulerAtClipFraction(clipId, 0.5);
}

async function seekTimelineAtClipFraction(
  clipId: string,
  fraction: number,
...
async function readPlayheadUiState() {
  return executeSync<PlayheadUiState>(`
...
async function setControlValue(selector: string, value: string | number) {
  const result = await executeSync<{ value: string; disabled: boolean } | null>(`
...
async function readStoredProject(): Promise<ProjectRecord | null> {
  const result = await executeAsync<{ error?: unknown; value?: unknown }>(`
  ...
  assert(
    isProjectRecord(result.value),
    `IndexedDB 프로젝트 shape가 올바르지 않습니다: ${
      JSON.stringify(result.value).slice(0, 2_000)
    }`
  );
  return result.value;
}
...
function requireClip(project: ProjectRecord, clipId: string): ClipRecord {
  const clip = project.clips.find((candidate) => candidate.id === clipId);
  assert(clip, `프로젝트에서 ${clipId} 컷을 찾지 못했습니다.`);
  return clip;
}

async function installMemoryDirectoryPicker(
  preexistingFiles: Array<{ name: string; contents: string }> = []
) {
  return executeAsync<PickerOverride>(`
...
async function captureMemoryDirectoryFiles() {
  return executeAsync<MemoryDirectorySnapshot>(`
    const done = arguments[arguments.length - 1];
    const state = globalThis.__kirinukiE2eDirectory;
    if (!state) {
...
async function probeMedia(ffprobe: string, filePath: string): Promise<MediaProbe> {
...
async function sampleFrameRgb(ffmpeg: string, filePath: string, timestamp: number) {
...
async function sampleFrameRgbPixels(
  ffmpeg: string,
  filePath: string,
  timestamp: number
): Promise<Buffer> {
...
type CaptionColorSample = {
  count: number;
  centerX: number | null;
  centerY: number | null;
};

function analyzeCaptionColors(pixels: Buffer): Record<"red" | "green", CaptionColorSample> {
  const result = {
  ...
  ])) as Record<"red" | "green", CaptionColorSample>;
}

function averageRgbRegion(pixels: Buffer, {
  x,
  y,
  width = 12,
  height = 12
...
function audioRms(samples: number[]) {
  return Math.sqrt(
...
function tonePower(samples: number[], sampleRate: number, frequency: number) {
  let real = 0;
  let imaginary = 0;
...
async function main() {
  ...
  assert(created.sessionId, "ChromeDriver session ID를 받지 못했습니다.");
  ...
  assert(attachedProject.mediaAsset, "연결 완료 프로젝트에 mediaAsset이 없습니다.");
  ...
  const imageAssetProject = await waitForStoredProject((project) => {
    const asset = project.imageAssets?.find(
  ...
  assert(imageAsset, "복원된 투명 PNG 에셋을 찾지 못했습니다.");

  const previewPaused = await executeSync<boolean>(
  ...
  `);
  const safePointerTargets: ExternalRecord[] = [];
  ...
  let safePlayheadObserved: ExternalRecord | null = null;
  const safePlayhead = await waitUntil(async () => {
    ...
    throw new Error(
      `${error instanceof Error ? error.message : String(error)} 관측값: `
      + JSON.stringify(safePlayheadObserved)
    );
  });
  ...
  const audioUiActions: ExternalRecord = {};
  ...
  assert(
    pickerOverride?.ok &&
      ...
      pickerOverride.contractProbe?.operations?.map(
        (operation: ExternalRecord) => operation.type
      ).join(",") ===
  ...
  const encoderDiagnostics = await executeAsync<{
    videoEncoder: string;
    audioEncoder: string;
    results: Array<{ name: string; supported?: boolean; error?: string }>;
  }>(`
    const done = arguments[arguments.length - 1];
    const videoConfigs = [
      {
  ...
  const encoderSupport = Object.fromEntries(
    encoderDiagnostics.results.map(
      (result) => [result.name, result.supported === true]
    )
  );
  ...
  assert(
    (() => {
      ...
      const lookupCalls = memoryDirectory.getFileHandleCalls.filter(
        (call: ExternalRecord) => !call.create
      );
      const createCalls = memoryDirectory.getFileHandleCalls.filter(
        (call: ExternalRecord) => call.create
      );
      ...
      return lookupCalls.length === expectedLookups.length &&
        ...
        lookupCalls.map((call: ExternalRecord) => call.name).join(",") ===
          expectedLookups.join(",") &&
        createCalls.map((call: ExternalRecord) => call.name).join(",") ===
          expectedCreates.join(",");
    })(),
  ...
  const expectedSrt = captionCues.map((cue, index) => {
    const clip = requireClip(captionProject, cue.clipId);
    return [
      index + 1,
  ...
  const videoStream = outputProbe.streams.find(
    (stream: ExternalRecord) => stream.codec_type === "video"
  );
  const audioStream = outputProbe.streams.find(
    (stream: ExternalRecord) => stream.codec_type === "audio"
  );
  ...
  const gainTimelineStart = (
    requireClip(audioProject, gainRegion.clipId).timelineStartMs + gainRegion.startOffsetMs
  ) / 1_000;
  const mutedTimelineStart = (
    requireClip(audioProject, mutedRegion.clipId).timelineStartMs + mutedRegion.startOffsetMs
  ) / 1_000;
  const mutedTimelineEnd = (
    requireClip(audioProject, mutedRegion.clipId).timelineStartMs + mutedRegion.endOffsetMs
  ) / 1_000;
  const fadeTimelineStart = (
    requireClip(audioProject, fadeRegion.clipId).timelineStartMs + fadeRegion.startOffsetMs
  ) / 1_000;
  const fadeTimelineEnd = (
    requireClip(audioProject, fadeRegion.clipId).timelineStartMs + fadeRegion.endOffsetMs
  ) / 1_000;
  const [
    firstColor,
    secondColor,
  ...
  const assetCompositeWithDelta = {
    ...assetComposite,
    transparentBackgroundDelta
  };
  ...
  assert(
    captionColorAnalysis.red.count >= 40 &&
      ...
      captionColorAnalysis.red.centerY !== null &&
      captionColorAnalysis.green.centerY !== null &&
  ...
  console.log(JSON.stringify({
    ...
    project: {
      ...
      durationMs: reorderedProject.clips.reduce(
        (total, clip) => (
          total + clip.sourceEndMs - clip.sourceStartMs
        ),
        0
      ),
      timelineUi: {
    ...
    output: {
      ...
      firstColor,
      secondColor,
      assetComposite: assetCompositeWithDelta,
