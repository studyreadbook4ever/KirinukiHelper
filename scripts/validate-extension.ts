...
assert(!manifest.host_permissions?.some((permission: string) => permission.includes("huggingface") || permission.includes("hf.co")), "삭제한 로컬 모델의 Hugging Face permission이 남아 있습니다.");
...
for (const relativePath of [
  ...
  "editor/vendor/ort-wasm-simd-threaded.jsep.js",
  ...
  try {
    ...
    if (!hasErrorCode(error, "ENOENT")) {
      errors.push(`삭제 파일 확인 실패: ${relativePath} (${errorMessage(error)})`);
    }
  }
}
...
try {
  ...
  if (!hasErrorCode(error, "ENOENT")) {
    errors.push(`개발 marker 확인 실패: ${errorMessage(error)}`);
  }
}
...
for (const entry of await readdir(extensionRoot)) {
  ...
  try {
    ...
    if (!hasErrorCode(error, "ENOENT")) {
      errors.push(`개발 임시 marker 정리 실패: ${entry} (${errorMessage(error)})`);
    }
  }
}
...
for (const entry of await readdir(root)) {
  ...
  try {
    ...
    if (!hasErrorCode(error, "ENOENT")) {
      errors.push(
        `이전 개발 임시 lock 정리 실패: ${entry} (${errorMessage(error)})`
      );
    }
  }
}
...
for (const [label, relativePath, expectedSha256] of [
  ...
  try {
    ...
    if (!hasErrorCode(error, "ENOENT")) {
      errors.push(
        `${label} 배포 파일 읽기 실패: ${relativePath} (${errorMessage(error)})`
      );
    }
    continue;
  }
...
const creatorPolicies = Array.isArray(policyIndex.policies) ? policyIndex.policies : [];
...
try {
  ...
  errors.push(`프롬프트 smoke test 실패: ${errorMessage(error)}`);
}
