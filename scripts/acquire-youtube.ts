...
export type YouTubeAcquisitionProfile =
  keyof typeof YOUTUBE_ACQUISITION_PROFILES;
...
export function normalizeYouTubeVideoUrl(value: unknown): string {
  let url: URL;
  try {
...
export function parseAcquireYouTubeArgs(
  argv: readonly string[],
  { cwd = process.cwd() }: { cwd?: string } = {}
) {
  let profile: string = DEFAULT_ACQUISITION_PROFILE;
  let outputDir = cwd;
  let help = false;
  const positional = [];
  ...
  if (help) {
    return {
      help: true,
      profile: profile as YouTubeAcquisitionProfile,
...
export function assertNoMediaReencodeFlags(args: readonly string[]) {
...
export function buildYtDlpArgs({
  ...
  url: string;
  profile?: YouTubeAcquisitionProfile;
  outputDir?: string;
...
export async function runSpawnedCommand(command: string, args: readonly string[], {
  cwd = process.cwd(),
  stdio = "inherit",
...
export async function requireExecutable(command: string, {
  label,
  versionArgs,
  installHint,
  spawnImpl = spawn as SpawnImplementation
...
export async function runAcquireYouTubeCli(
