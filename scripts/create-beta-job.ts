...
const expectedById = Object.fromEntries(
  reference.expectedSessions.map((session) => [session.id, session])
);
...
const manifestBase = buildCodexJobManifest({
  projectName,
  source,
  globalInstruction,
  segments,
...
const manifest = {
  ...
  inputs: {
    ...manifestBase.inputs,
    fullVideo: {
      ...manifestBase.inputs.fullVideo,
      status: "PRESENT",
      fileName: path.basename(videoPath),
      sha256: reference.video.sha256
    }
  }
};
