...
const EDITOR_SCHEMA = "chzzk-kirinuki-editor/v3";
const EDITOR_PROJECTS_STORE_KEY = "chzzkKirinukiEditorProjectsV1";
const EDITOR_SEED_PREFIX = "chzzkKirinukiEditorSeed:";
const EDITOR_DATABASE_NAME = "chzzk-kirinuki-studio";
const MIN_SUBTITLE_LANES = 2;
const MAX_SUBTITLE_LANES = 8;
const MAX_AI_WARNINGS = 4e3;
const MAX_AI_CAPTION_CHECKPOINTS = 500;
const DEFAULT_SUBTITLE_COLOR = "#ffffff";
const MAX_RECENT_SUBTITLE_COLORS = 5;
const SUPPORTED_IMAGE_ASSET_MIME_TYPES = Object.freeze([
  "image/png",
  "image/jpeg",
  "image/webp",
  "image/gif"
...
const ACCEPTED_EDITOR_SCHEMAS = /* @__PURE__ */ new Set([
  EDITOR_SCHEMA,
...
const secondsToMilliseconds = (seconds) => Math.max(0, Math.round(finiteNumber(seconds) * 1e3));
const millisecondsToSeconds = (milliseconds) => Math.max(0, finiteNumber(milliseconds) / 1e3);
...
function normalizeAiWarnings(value) {
  const source = Array.isArray(value) ? value : [];
  const warnings = [];
  let truncated = source.length > MAX_AI_WARNINGS;
  for (let index = 0; index < Math.min(source.length, MAX_AI_WARNINGS + 1); index += 1) {
...
function mergeAiWarnings(existing, incoming, clipId) {
  const normalizedExisting = normalizeAiWarnings(existing);
  if (normalizedExisting.at(-1)?.code === "TRIMMED_WARNING_COUNT") {
    return normalizedExisting;
  }
  const boundedIncoming = (Array.isArray(incoming) ? incoming : []).slice(0, MAX_AI_WARNINGS + 1).map((warning) => ({
    ...warning,
...
function normalizeAiCaptionCheckpoints(value, clips = []) {
  ...
  for (const raw of (Array.isArray(value) ? value : []).slice(
    ...
    if (!clipId || !clipIds.has(clipId) || sourceStartMs < 0 || sourceEndMs <= sourceStartMs || !["whisper-tiny", "audseg-local"].includes(model)) {
      continue;
    }
...
function rememberSubtitleColor(project, rawColor) {
  ...
  if (!color || color === DEFAULT_SUBTITLE_COLOR) {
    return Array.isArray(project.recentSubtitleColors) && project.recentSubtitleColors.length === current.length && project.recentSubtitleColors.every((entry, index) => entry === current[index]) ? project : { ...project, recentSubtitleColors: current };
  }
  const next = [
    color,
    ...current.filter((candidate) => candidate !== color)
  ].slice(0, MAX_RECENT_SUBTITLE_COLORS);
  if (Array.isArray(project.recentSubtitleColors) && project.recentSubtitleColors.length === next.length && project.recentSubtitleColors.every((entry, index) => entry === next[index])) {
    return project;
  }
  return {
    ...project,
...
function sameSourceSession(leftSource = {}, rightSource = {}) {
  const leftPlatform = String(leftSource.platform ?? "CHZZK").trim().toUpperCase() || "CHZZK";
  const rightPlatform = String(rightSource.platform ?? "CHZZK").trim().toUpperCase() || "CHZZK";
  if (leftPlatform !== rightPlatform) {
    return false;
  }
  const leftContentType = String(leftSource.contentType ?? "unknown").trim().toLowerCase() || "unknown";
  const rightContentType = String(rightSource.contentType ?? "unknown").trim().toLowerCase() || "unknown";
  const sameContentType = leftContentType === rightContentType;
  const chzzkLiveVodPair = leftPlatform === "CHZZK" && (/* @__PURE__ */ new Set([leftContentType, rightContentType])).size === 2 && [leftContentType, rightContentType].every((type) => type === "live" || type === "vod");
  ...
  const involvesChzzkLive = leftPlatform === "CHZZK" && (leftContentType === "live" || rightContentType === "live");
  ...
  if (leftPlatform === "CHZZK") {
    ...
    if (leftChannelId && rightChannelId && leftStartedAt && rightStartedAt) {
      return leftChannelId === rightChannelId && leftStartedAt === rightStartedAt;
    }
    if (chzzkLiveVodPair) {
      return false;
    }
    if (leftContentType === "live") {
      return Boolean(
        leftChannelId && rightChannelId && leftChannelId === rightChannelId && !leftStartedAt && !rightStartedAt
      );
    }
  }
...
function captureStateSourceConflict(captureState = {}, nextSource = {}) {
  ...
  const hasRange = Boolean(
    Array.isArray(captureState.segments) && captureState.segments.length > 0 || draft.startCapture || draft.endCapture || String(draft.startText || "").trim() || String(draft.endText || "").trim()
  );
  return Boolean(
    hasRange && previousIdentity && nextIdentity && !sameSourceSession(captureState.source, nextSource)
  );
}
function captureProjectId(captureState = {}) {
...
function mergeCaptureIntoEditorProject(project, captureState = {}) {
...
function projectDurationMs(project) {
  return (project?.clips || []).reduce((total, clip) => clip.enabled === false ? total : total + Math.max(0, clip.sourceEndMs - clip.sourceStartMs), 0);
}
function clipDurationMs(clip) {
  return Math.max(0, finiteNumber(clip?.sourceEndMs) - finiteNumber(clip?.sourceStartMs));
}
function mapTimelineToSource(project, timelineMs) {
  const enabled = (project?.clips || []).filter((clip2) => clip2.enabled !== false);
  if (enabled.length === 0) {
    return null;
  }
...
function mapSourceToTimeline(project, clipId, sourceMs) {
...
function resetAiSubtitlePositions(project, {
...
function createSubtitleCue(project, {
  id,
  clipId,
  startOffsetMs = 0,
  endOffsetMs = 2e3,
...
function createImageAsset(project, {
  id,
  clipId,
  startOffsetMs = 0,
  endOffsetMs = 2e3,
...
function updateImageAsset(project, assetId, patch = {}) {
...
function deleteImageAsset(project, assetId) {
  return {
    ...project,
...
function createAudioRegion(project, {
  id,
  clipId,
  startOffsetMs = 0,
  endOffsetMs = 2e3,
...
function updateAudioRegion(project, regionId, patch = {}) {
...
function deleteAudioRegion(project, regionId) {
  return {
    ...project,
...
function cueTimelineRange(project, cue) {
...
function audioRegionTimelineRange(project, region) {
...
function timelineSnapThresholdMs(pixelsPerSecond, {
  thresholdPx = 8,
  minimumMs = 25,
  maximumMs = 400
} = {}) {
...
function timelineSnapCandidates(project, {
  clipId,
  excludeCueId = null,
...
function matchSubtitleCueToImageAsset(project, cueId, assetId) {
...
function matchImageAssetToSubtitleCue(project, assetId, cueId) {
  const asset = project?.imageAssets?.find((candidate) => candidate.id === assetId);
  const cue = project?.subtitles?.find((candidate) => candidate.id === cueId);
...
function cueAtTimeline(project, timelineMs) {
  return cuesAtTimeline(project, timelineMs)[0] || null;
}
function cuesAtTimeline(project, timelineMs) {
  const target = Math.round(finiteNumber(timelineMs));
  return (project?.subtitles || []).map((cue) => ({ cue, range: cueTimelineRange(project, cue) })).filter(hasTimelineRange).filter(({ range }) => target >= range.startMs && target < range.endMs).sort((a, b) => a.cue.lane - b.cue.lane || a.range.startMs - b.range.startMs || a.cue.id.localeCompare(b.cue.id)).map(({ cue }) => cue);
}
function imageAssetsAtTimeline(project, timelineMs) {
  const target = Math.round(finiteNumber(timelineMs));
...
function findImageAssetOverlaps(project) {
  const assets = (project?.imageAssets || []).map((asset, order) => ({
    asset,
    order,
    range: imageAssetTimelineRange(project, asset)
  })).filter(hasTimelineRange).sort((a, b) => a.range.startMs - b.range.startMs || a.order - b.order);
  const overlaps = [];
...
function findSubtitleOverlaps(project) {
  const cues = (project?.subtitles || []).map((cue) => ({ cue, range: cueTimelineRange(project, cue) })).filter(hasTimelineRange).sort((a, b) => a.range.startMs - b.range.startMs);
  const overlaps = [];
...
function audioRegionAtTimeline(project, timelineMs) {
  const target = Math.round(finiteNumber(timelineMs));
  return (project?.audioRegions || []).map((region) => ({ region, range: audioRegionTimelineRange(project, region) })).find(({ range }) => range && target >= range.startMs && target < range.endMs)?.region || null;
}
function findAudioRegionOverlaps(project) {
  const regions = (project?.audioRegions || []).map((region) => ({ region, range: audioRegionTimelineRange(project, region) })).filter(hasTimelineRange).sort((a, b) => a.range.startMs - b.range.startMs);
  const overlaps = [];
...
function addSubtitleLane(project) {
  const subtitleLaneCount = clamp(
...
function updateSubtitleCue(project, cueId, patch = {}, { markHuman = true } = {}) {
...
function deleteSubtitleCue(project, cueId) {
  return {
    ...project,
...
function isAudSegBlankTimingCue(cue) {
  return Boolean(
    cue?.origin === "ai" && cue.remoteMeta?.qualityCodes?.includes("AUDSEG_BLANK_TIMING")
  );
}
function subtitleCueNeedsReview(cue) {
  return Boolean(
    cue?.origin === "ai" && cue.remoteMeta?.reviewRequired && (!cue.humanEdited || isAudSegBlankTimingCue(cue) && !String(cue.text || "").trim())
  );
}
const PRIMARY_AI_SPEAKER_IDS = /* @__PURE__ */ new Set([
  "",
  "host",
  "main",
  "primary",
...
function replaceAiSubtitleDraft(project, clipId, drafts = []) {
  const clip = project.clips.find((candidate) => candidate.id === clipId);
  if (!clip) {
    return project;
  }
  const preserved = project.subtitles.filter((cue) => cue.clipId !== clipId || cue.origin !== "ai" || cue.humanEdited);
  const protectedInClip = preserved.filter((cue) => cue.clipId === clipId);
  const normalizedDrafts = drafts.filter((draft) => String(draft?.text || "").trim()).map((draft) => createSubtitleCue(project, {
    ...draft,
    clipId,
    lane: 0,
    origin: "ai"
  ...
  const speakerLanes = /* @__PURE__ */ new Map();
...
function replaceAiBlankTimingDraft(project, clipId, drafts = []) {
  ...
  const protectedRangeSignatures = new Set(
    project.subtitles.filter((cue) => cue.clipId === clipId && cue.origin === "ai" && cue.humanEdited && cue.remoteMeta?.qualityCodes?.includes("AUDSEG_BLANK_TIMING")).map(rangeSignature).filter(Boolean)
  );
  const timingDrafts = (Array.isArray(drafts) ? drafts : []).filter((draft) => Number.isFinite(Number(draft?.startOffsetMs)) && Number.isFinite(Number(draft?.endOffsetMs)) && Number(draft.endOffsetMs) > Number(draft.startOffsetMs) && !protectedRangeSignatures.has(rangeSignature(draft))).map((draft) => ({
    ...draft,
    text: "\u2026"
  }));
  ...
  return {
    ...
    subtitles: withVisiblePlaceholders.subtitles.map((cue) => cue.clipId === clipId && cue.origin === "ai" && !cue.humanEdited && cue.text === "\u2026" && cue.remoteMeta?.qualityCodes?.includes("AUDSEG_BLANK_TIMING") ? { ...cue, text: "" } : cue)
  };
}
function appendAiSubtitleDrafts(project, drafts = []) {
...
function transcriptChunksToCueDrafts(chunks = [], clipDuration = 0, {
  maxCharacters = 26,
  maxDurationMs = 4e3,
  gapBreakMs = 800,
  minimumDurationMs = 650
} = {}) {
  const clipDurationMs2 = Math.max(MIN_CUE_DURATION_MS, Math.round(finiteNumber(clipDuration)));
  const boundedMaxDurationMs = Math.max(
    MIN_CUE_DURATION_MS,
    Math.round(finiteNumber(maxDurationMs, 4e3))
  );
  const words = chunks.flatMap((chunk) => {
    ...
    const end = rawEnd === null || rawEnd === void 0 ? Math.min(clipDurationMs2, start + minimumDurationMs) : secondsToMilliseconds(rawEnd);
    const startMs = clamp(
      start,
      0,
      Math.max(0, clipDurationMs2 - MIN_CUE_DURATION_MS)
    );
    return [{
      text,
      startMs,
  ...
  const flush = () => {
    ...
    const paddedEnd = Math.min(
      ...
      nextStart === void 0 ? clipDurationMs2 : Math.max(naturalEnd, nextStart - 40)
    );
    drafts.push({
      startOffsetMs,
  ...
  const splitTextIntoParts = (text, requestedParts) => {
    ...
    const units = wordsInText.length >= requestedParts ? wordsInText : Array.from(text);
  ...
  const durationBoundedDrafts = drafts.flatMap((draft) => {
    ...
    return textParts.map((text, index) => {
      const gapBeforeMs = textParts.length <= 1 ? 0 : availableGapMs * index / (textParts.length - 1);
...
function updateClipTrim(project, clipId, {
  sourceStartMs,
  sourceEndMs
} = {}) {
...
function rippleDeleteTimelineRange(project, {
  startMs,
  endMs
} = {}) {
  ...
  const selectedClipId = typeof project.selectedClipId === "string" && clips.some((clip) => clip.id === project.selectedClipId) ? project.selectedClipId : mapTimelineToSource(projectWithClips, playheadMs)?.clipId || clips[0]?.id || null;
...
function applyMediaAlignmentOffset(project, alignmentOffsetMs) {
...
function serializeSrt(project) {
  const cues = (project?.subtitles || []).map((cue) => ({ cue, range: cueTimelineRange(project, cue) })).filter(hasTimelineRange).filter(({ cue }) => cue.text.trim()).sort((a, b) => a.range.startMs - b.range.startMs);
