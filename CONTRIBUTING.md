...
PC에 준비하는 로컬 미디어 앱을 제공합니다. 설치 앱이 소유한 격리된 Electron
창은 컷 선택에만 쓰이고, 완성 편집기는 일반 브라우저에서 열립니다. 기여는 이
제품 경계와 [개발 계약](AGENTS.md)을 먼저 지켜야 합니다.
...
- 플레이어 제어를 별도 extension·`extraResource`로 내보내거나 full editor를
  Electron 안으로 옮기지 마세요. 고정 코드는 `app.asar`에 포함합니다.
