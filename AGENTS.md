...
예전 Linux 소스 앱, 브라우저 확장 프로그램, Electron 편집기 창, 사용자가 직접
주소·포트·프로세스를 관리하는 흐름은 제품 계약이 아니다.

## 제품 한 문장

Kirinuki는 `https://kirinuki.eff0rtchung.kr`에서 실행되는 전체 웹 편집기와,
Windows x64·macOS Apple Silicon·Linux x64에 처음 한 번 설치하는 화면 없는
로컬 미디어 엔진으로 구성된다.

일반 사용자는 다음 흐름만 경험해야 한다.

1. 웹사이트에 CHZZK·YouTube·SOOP 공개 완료 VOD URL을 붙여넣는다.
2. 원본 시각 기준 시작·끝을 정하고 편집기를 연다.
3. **편집 영상 준비**를 누른다.
4. 엔진이 없을 때만 OS에 맞는 설치 안내를 한 번 따른다.
5. 같은 웹페이지가 엔진을 자동 감지하고 필요한 구간만 이 PC에 준비한다.
6. 컷·자막·레이어를 검수하고 제목과 저장 위치를 정해 내보낸다.

설치 뒤 사용자가 로컬 주소, 포트, 토큰, 실행·종료 상태, 별도 편집 창 또는
브라우저 확장을 보거나 조작해야 한다면 제품 회귀다. Chrome이 최초 연결에서
표시하는 Local Network Access 권한은 운영체제·브라우저가 요구하는 한 번의
온보딩으로 설명하고, 그 뒤에는 일반 웹사이트처럼 동작해야 한다.

## 현재 배포 경계

```text
https://kirinuki.eff0rtchung.kr
  └─ 시작 화면·full editor·컷·자막·렌더 (브라우저)
...
- Electron은 세 OS의 동일한 background engine을 패키징하는 런타임일 뿐이다.
  `BrowserWindow`나 별도 편집 UI를 제품에 넣지 않는다.
...
- `src/lib/`: source, project, time mapping, session, capability 공용 계약
- `src/desktop/`: 화면 없는 엔진 entry, autostart, 단일 인스턴스, installer 계약
...
삭제된 extension, Linux helper, Electron editor 또는 Origin migration 파일을
문서·build·test inventory에 다시 넣지 않는다. 파일이나 명령 이름이 바뀌면 이
지도를 같은 변경에서 갱신한다.
...
suppression, generated bundle provenance와 삭제된 extension build 경계의 재유입을
...
- [ ] 제품 본체가 public HTTPS full editor이고 설치기는 화면 없는 엔진뿐임
...
- [ ] 브라우저 확장·Electron 편집 창·서버 VOD proxy·login·analytics가 없음
