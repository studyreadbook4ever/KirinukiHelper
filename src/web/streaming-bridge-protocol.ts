...
export const STREAMING_BRIDGE_SHORTCUT = "KIRINUKI_STREAMING_BRIDGE_SHORTCUT";
...
export const STREAMING_BRIDGE_SHORTCUT_KEYS = Object.freeze([
  "Q", "W", "E", "R", "T", "D", "F", "Y", "U", "A", "S"
] as const);
export type StreamingBridgeShortcutKey =
  typeof STREAMING_BRIDGE_SHORTCUT_KEYS[number];
...
export interface StreamingBridgeShortcutMessage {
  readonly protocol: typeof STREAMING_BRIDGE_PROTOCOL;
  readonly type: typeof STREAMING_BRIDGE_SHORTCUT;
  readonly eventId: string;
  readonly generation: number;
  readonly source: StreamingBridgeSourceIdentity;
  readonly key: StreamingBridgeShortcutKey;
}
...
export function parseStreamingBridgeShortcutMessage(
  value: unknown
): StreamingBridgeShortcutMessage | null {
  if (
    !isRecord(value)
    || !hasExactKeys(value, [
      "protocol", "type", "eventId", "generation", "source", "key"
    ])
    || value.protocol !== STREAMING_BRIDGE_PROTOCOL
    || value.type !== STREAMING_BRIDGE_SHORTCUT
  ) {
    return null;
  }
  const eventId = normalizedRequestId(value.eventId);
  const generation = normalizedGeneration(value.generation);
  const source = parseIdentity(value.source);
  const key = boundedPlainString(value.key, 1).toUpperCase();
  if (
    !eventId
    || !generation
    || !source
    || !STREAMING_BRIDGE_SHORTCUT_KEYS.includes(
  ...
  return {
    ...
    generation,
    source,
    key: key as StreamingBridgeShortcutKey
  };
}
