...
const MAXIMUM_QUEUED_STREAMING_BRIDGE_ACTIONS = 32;
...
export class StreamingBridgeClient {
  ...
  destroy(): void {
    ...
    if (this.#queuedActions >= MAXIMUM_QUEUED_STREAMING_BRIDGE_ACTIONS) {
      return Promise.reject(new StreamingBridgeRequestError(
        "bridge-busy",
        "스트리밍 제어 요청이 너무 많이 대기 중입니다. 잠시 뒤 다시 시도해 주세요."
      ));
    }
