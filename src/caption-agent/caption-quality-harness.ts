...
export interface CaptionQualityProfile {
  ...
  maxLineWidthUnits: number;
  maxTotalWidthUnits: number;
  maxReadingRateUnitsPerSecond: number;
  minimumTranscriptCoverage: number;
  minimumTranscriptPrecision: number;
  defaultPlacement: string;
  automaticTopPlacement: boolean;
  topPlacementEvidence: {
    minimumSamples: number;
    minimumMeanAdvantage: number;
    minimumSampleAdvantage: number;
    minimumWinningRatio: number;
  };
  speakerPalette: readonly string[];
}

export interface CaptionQualityWarning {
  code: string;
  cueIndex: number;
}

interface WarningCollector {
  add(code: string, cueIndex: unknown): void;
  values(): CaptionQualityWarning[];
}

export interface CanonicalTranscriptUnit {
  startMs: number;
  endMs: number;
  text: string;
  speakerId: string;
  wordAnchors?: Array<[number, number]>;
}

export interface CanonicalTranscript {
  units: CanonicalTranscriptUnit[];
  wordUnits: CanonicalTranscriptUnit[];
  text: string;
  anchorCoverage: {
    segmentTextCoverage: number;
    wordTextPrecision: number;
  };
  warnings: CaptionQualityWarning[];
}

export interface CaptionHarnessCue {
  startMs: number;
  endMs: number;
  text: string;
  speakerId: string;
...
interface WorkingCaptionCue extends CaptionHarnessCue {
...
interface CueMetrics {
  ...
  transcriptPrecision: number | null;
}

export interface CaptionQualityMetrics {
  cueCount: number;
  maximumDurationMs: number;
  maximumLineWidthUnits: number;
  maximumTotalWidthUnits: number;
  maximumReadingRate: number | null;
  transcriptCoverage: number | null;
  transcriptPrecision: number | null;
...
export interface CaptionQualityReport {
  ...
  valid: boolean;
  disposition: "accepted" | "review-required" | "rejected";
...
let koreanGraphemeSegmenter: Intl.Segmenter | undefined;
...
function graphemes(value: unknown): string[] {
...
function normalizeSpeaker(value: unknown, editorialContext?: unknown): string {
  return canonicalCaptionSpeakerId(value, editorialContext);
}

export function normalizeCaptionDisplayText(value: unknown): string {
  return String(value ?? "")
    .replace(/\s+/gu, " ")
    .trim()
...
export function measureKoreanWidthUnits(value: unknown): number {
...
function warningCollector(): WarningCollector {
  const warnings: CaptionQualityWarning[] = [];
  const seen = new Set<string>();
  return {
    add(code: string, cueIndex: unknown): void {
      const normalizedCueIndex = typeof cueIndex === "number"
...
function candidateTranscriptRange(
...
function sortAndDeduplicateTranscriptUnits(
  units: CanonicalTranscriptUnit[],
  collector: WarningCollector
...
function transcriptAnchorCoverage(
  segmentUnits: CanonicalTranscriptUnit[],
  wordUnits: CanonicalTranscriptUnit[]
): { segmentTextCoverage: number; wordTextPrecision: number } {
...
export function canonicalizeTranscriptUnits(transcript: unknown, {
  clipDurationMs,
  editorialContext
}: {
  ...
  const source: DynamicRecord = Array.isArray(transcript)
    ? { units: transcript }
    : transcript && typeof transcript === "object"
      ? transcript as DynamicRecord
      : {};
  ...
  const segmentUnits = sortAndDeduplicateTranscriptUnits(
    rawSegments.map((candidate, unitIndex) => normalizeTranscriptCandidate(
      ...
      collector,
      context
    )).filter((unit): unit is CanonicalTranscriptUnit => unit !== null),
    collector
  );
  ...
  const units = primaryUnits.map((unit) => {
    ...
    }).map(({ startMs, endMs }): [number, number] => [startMs, endMs]);
  ...
  const fullText = normalizeCaptionDisplayText(
    transcript && !Array.isArray(transcript)
      ? (transcript as DynamicRecord).text
      : ""
  );
  return {
    units,
...
function isPreferredBreak(
  graphemeBefore: string,
  graphemeAfter: string
): boolean {
  return (
...
function balancedTextBreak(value: unknown, {
  maxLineWidthUnits = Number.POSITIVE_INFINITY
}: { maxLineWidthUnits?: number } = {}): [string, string] | null {
...
function layoutCueText(text: unknown, profile: CaptionQualityProfile): string {
...
function normalizedRawCueRange(
  rawCue: DynamicRecord | null | undefined,
  clipDurationMs: number
): { startMs: number; endMs: number; repaired: boolean } {
...
function splitRawCue(
  ...
  collector: WarningCollector,
  {
    ...
    wordUnits?: CanonicalTranscriptUnit[];
    editorialContext?: unknown;
    preserveTimedBoundaries?: boolean;
  } = {}
): WorkingCaptionCue[] {
...
function visualSamples(visualPlacement: unknown): DynamicRecord[] {
  if (Array.isArray(visualPlacement)) {
    return visualPlacement;
  }
  const source = visualPlacement as DynamicRecord | null | undefined;
  return Array.isArray(source?.samples)
    ? source.samples
    : [];
}

export function chooseStableCaptionPlacement(visualPlacement: unknown, {
  profile = KR_VTUBER_CLEAN_PROFILE
}: { profile?: CaptionQualityProfile } = {}): string {
...
function repairSameSpeakerOverlaps(
  cues: WorkingCaptionCue[],
  collector: WarningCollector,
  {
  preserveTimedBoundaries = false
  }: { preserveTimedBoundaries?: boolean } = {}
): void {
  const bySpeaker = new Map<string, WorkingCaptionCue[]>();
...
function expandShortCuesWithoutSpeakerOverlap(
  cues: WorkingCaptionCue[],
  clipDurationMs: number,
  profile: CaptionQualityProfile,
  collector: WarningCollector
...
function cueTranscriptMetrics(
  cue: DynamicRecord,
  transcriptUnits: CanonicalTranscriptUnit[]
): { coverage: number; precision: number } {
  const cueStartMs = Number(cue.startMs);
  const cueEndMs = Number(cue.endMs);
  const expectedText = transcriptUnits.filter((unit) => (
    unit.endMs > cueStartMs && unit.startMs < cueEndMs
  )).map(({ text }) => text).join(" ");
...
export function evaluateCaptionCues(cues: unknown, {
  clipDurationMs,
  transcript,
  visualPlacement,
  editorialContext,
  ...
  const normalizedCues = cues as DynamicRecord[];
  ...
  const rangesBySpeaker = new Map<string, Array<{
    startMs: number;
    endMs: number;
    cueIndex: number;
  }>>();
  const perCueMetrics: CueMetrics[] = [];
  ...
  if (canonicalTranscript != null) {
    ...
    const actual = textSignature(
      normalizedCues.map((cue) => cue?.text || "").join(" ")
    );
    if (expected || actual) {
  ...
  const cueReviews: CaptionQualityReport["cueReviews"] = normalizedCues.map((
    cue,
    cueIndex
  ) => {
...
function deterministicSpeakerColors(
  cues: CaptionHarnessCue[],
  profile: CaptionQualityProfile
...
export function repairCaptionCues(rawCues: unknown, {
  clipDurationMs,
  transcript,
  visualPlacement,
  editorialContext,
...
export function repairCaptionDraft(
  rawCues: unknown,
  options: Parameters<typeof repairCaptionCues>[1]
) {
