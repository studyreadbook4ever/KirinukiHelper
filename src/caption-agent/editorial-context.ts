...
export interface CaptionEditorialContext {
  schema: typeof CAPTION_EDITORIAL_CONTEXT_SCHEMA;
  glossary: Array<{ term: string; variants: string[] }>;
  speakers: Array<{ id: string; aliases: string[] }>;
  style: {
    terminalPeriod: "omit";
    placement: "bottom";
    maxWidthUnits: 20;
...
interface EditorialCue {
  ...
  speakerId?: unknown;
  speaker?: unknown;
  remoteMeta?: { speakerId?: unknown } | null;
}

interface EditorialProject {
  clips?: Array<{ id?: unknown }>;
  subtitles?: EditorialCue[];
  ai?: { captionContext?: unknown } | null;
  source?: { streamerName?: unknown } | null;
}
...
export function romanizeHangulForIdentity(value: unknown): string {
  let output = "";
...
export function speakerIdentityKeys(value: unknown): string[] {
...
export function normalizeCaptionEditorialContext(raw?: unknown, {
  strict = false
}: { strict?: boolean } = {}): CaptionEditorialContext {
  if (raw == null || raw === "") {
    return {
  ...
  const glossary: CaptionEditorialContext["glossary"] = [];
  ...
  const style: CaptionEditorialContext["style"] = {
    terminalPeriod: "omit",
    placement: "bottom",
    maxWidthUnits: 20,
  ...
  const normalized: CaptionEditorialContext = {
    schema: CAPTION_EDITORIAL_CONTEXT_SCHEMA,
    glossary,
    speakers,
    style
...
export function canonicalCaptionSpeakerId(
  value: unknown,
...
function captionSpeakerId(cue: EditorialCue | null | undefined): string {
  return compactText(
...
function captionText(cue: EditorialCue | null | undefined): string {
  return compactText(cue?.text, 80)
    .replace(/[.\u3002\uff0e]+$/gu, "")
    .trim();
}

function projectTimelineOrder(
  project: EditorialProject | null | undefined,
  cue: EditorialCue | null | undefined
): number {
...
function trustedStyleExamples(project: EditorialProject | null | undefined): string[] {
...
function trustedGlossaryTerms(
  project: EditorialProject | null | undefined
): CaptionEditorialContext["glossary"] {
...
export function buildProjectCaptionEditorialContext(
  project: EditorialProject | null | undefined,
  {
  includeUnreviewedSpeakers = true
  }: { includeUnreviewedSpeakers?: boolean } = {}
): CaptionEditorialContext {
...
export function captionEditorialContextFingerprint(editorialContext?: unknown): string {
