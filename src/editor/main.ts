...
import type { LocalDraftRecord } from "./project-store.js";
import {
  AUDSEG_DRAFT_MODEL,
...
const projectSaveOperations = new Set<Promise<unknown>>();
...
function selectAudioRegion(regionId: string, { seek = false } = {}) {
...
function downloadSidecars(sidecars: ExportSidecar[]) {
...
function bindTimelineSeeking() {
  ...
  elements.timeline_ruler.addEventListener("pointerdown", (event) => {
    ...
    const move = (moveEvent: PointerEvent) => seekFromEvent(moveEvent);
    const finish = () => {
...
async function initialize() {
  ...
  let seedMergeError: Error | null = null;
  ...
  if (storedProject) {
    if (resumeSavedSession) {
      ...
      try {
        ...
        project = storedProject;
        seedMergeError = error instanceof Error
          ? error
...
function applyCaptureSeedUpdate(captureState: unknown) {
  try {
