...
function isMainSpeakerId(speakerId: unknown): boolean {
  return WHITE_SPEAKER_IDS.has(normalizedSpeakerId(speakerId));
}

export function normalizeCaptionStylePresetId(presetId: unknown): CaptionPresetId {
  const normalized = String(presetId || "").trim();
  return Object.hasOwn(CAPTION_STYLE_PRESETS, normalized)
    ? normalized as CaptionPresetId
    : DEFAULT_CAPTION_STYLE_PRESET_ID;
}

export function captionStylePreset(presetId: unknown = DEFAULT_CAPTION_STYLE_PRESET_ID) {
  return CAPTION_STYLE_PRESETS[normalizeCaptionStylePresetId(presetId)];
}

export function captionStyleDefaults(presetId: unknown = DEFAULT_CAPTION_STYLE_PRESET_ID) {
...
export function captionSpeakerColor(speakerId: unknown): string {
...
export function captionSpeakerColorAssignments(
  speakerIds: readonly unknown[] | unknown,
  existingAssignments: ReadonlyMap<unknown, unknown> | Record<string, unknown> = {}
): SpeakerColorAssignments {
  const assignments: SpeakerColorAssignments = {};
